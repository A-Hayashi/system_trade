...
class StockDataGetter
  ...
  def get_price_data_from_historical_data_pages
    ...
    begin
      ...
      begin
        text = open(url).read.encode(Encoding::UTF_8 ,:undef => :replace)
      rescue EOFError
        return[]
      end
      text.scan(reg_prices)
  ...
  def reg_prices
    %r!<td>(\d{4}年\d{1,2}月\d{1,2}日)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td></tr>!
  end
